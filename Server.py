...
def tratamento_de_mensagem(data, address):
    ...
    try:
        # Verifica se o cliente já está registrado (se o endereço está no dicionário `clients`)
        if address not in clients:
            # Novo cliente conectando
            username = data.decode().split("eh ")[1].strip()  # Extrai o nome do cliente
            clients[address] = username  # Adiciona o cliente no dicionário
            ...
            if message.lower() == 'bye':
                ...
                username = clients.pop(address, None)  # Remove o cliente do dicionário
                if username:
                    print(f"{username} saiu da sala.")
                    server.sendto(f"Você foi desconectado.".encode(), address)
